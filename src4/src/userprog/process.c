...
process_execute (const char *file_name) 
{
  ...
  //printf("In process_execute, filename is %s\n", file_name);
  struct file *open_file = filesys_open(filename); 
  if(open_file==NULL)
     return -1;
  //args-none fails above.
...
process_exit (void)
{
  ...
  //if(cur->workdir==NULL){
      //printf("eeeeeeeee\n");
  //}
  //printf("cur->workdir %x\n",cur->workdir);
  dir_close(cur->workdir);
  //printf("null?\n");
  sema_down(&cur->exit_sema);
  //printf("closing\n"); 
  /* Free thread's current working directory information */
  //if(cur->workdir != NULL){
    //printf("clo???????????\n");
    //dir_close(cur->workdir);
  //}
